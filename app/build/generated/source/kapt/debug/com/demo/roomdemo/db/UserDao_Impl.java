  ...
  public UserDao_Impl(RoomDatabase __db) {
    ...
    this.__insertionAdapterOfUserEntity = new EntityInsertionAdapter<UserEntity>(__db) {
      ...
      public String createQuery() {
        return "INSERT OR ABORT INTO `userinfo`(`id`,`name`,`email`,`phone`,`pais`) VALUES (nullif(?, 0),?,?,?,?)";
      }
      ...
      public void bind(SupportSQLiteStatement stmt, UserEntity value) {
        ...
        if (value.getPais() == null) {
          ...
          stmt.bindString(5, value.getPais());
        }
      }
    };
    ...
    this.__updateAdapterOfUserEntity = new EntityDeletionOrUpdateAdapter<UserEntity>(__db) {
      @Override
      public String createQuery() {
        return "UPDATE OR ABORT `userinfo` SET `id` = ?,`name` = ?,`email` = ?,`phone` = ?,`pais` = ? WHERE `id` = ?";
      }
  ...
  public List<UserEntity> getAllUserInfo() {
    ...
    try {
      ...
      while(_cursor.moveToNext()) {
        ...
        _item = new UserEntity(_tmpId,_tmpName,_tmpEmail,_tmpPhone,_tmpPais);
        _result.add(_item);
      }
      return _result;
    } finally {
